...
F)+", 0)"})}function l(a,b){a.attr("transform",function(a){return"translate(0,"+Math.ceil(b(a))+")"})}function q(a){var b=a[0];a=a[a.length-1];return b<a?[b,a]:[a,b]}function v(a){var b,e=[];if(a.ticks)return a.ticks.apply(a,u);b=a.domain();for(a=Math.ceil(b[0]);a<b[1];a++)e.push(a);0<e.length&&0<e[0]&&e.unshift(e[0]-(e[1]-e[0]));return e}function f(){var a=J.copy(),e;b.isCategory&&(e=J.domain(),a.domain([e[0],e[1]-1]));return a}function g(a){a=r?r(a):a;return"undefined"!==typeof a?a:""}function t(a){if(P)return P;
var b={h:11.5,w:5.5};a.select("text").text(g).each(function(a){var e=this.getBoundingClientRect(),l=g(a);a=e.height;e=l?e.width/l.length:void 0;a&&e&&(b.h=a,b.w=e)}).text("");return P=b}function y(r){r.each(function(){function r(a,e){function l(a,b){f=void 0;for(var q=1;q<b.length;q++)if(" "===b.charAt(q)&&(f=q),v=b.substr(0,q+1),p=S.w*v.length,e<p)return l(a.concat(b.substr(0,f?f:q)),b.slice(f?f+1:q));return a.concat(b)}var q=g(a),v,f,p;if("[object Array]"===Object.prototype.toString.call(q))return q;
if(!e||0>=e)e=W?95:b.isCategory?Math.ceil(M(V[1])-M(V[0]))-12:110;return l([],q+"")}function u(a,b){var e=S.h;0===b&&(e="left"===h||"right"===h?-(S.h/2*(N[a.index]-1)-3):".71em");return e}function ca(a){a=J(a)+(Y?0:F);return Q[0]<a&&a<Q[1]?6:0}var K=y.g=a.select(this),G=this.__chart__||J,M=this.__chart__=f(),V=n?n:v(M),w=K.selectAll(".tick").data(V,M),I=w.enter().insert("g",".domain").attr("class","tick").style("opacity",1E-6),x=w.exit().remove(),H=(b.withoutTransition?w:a.transition(w)).style("opacity",
1),O,R,A,Q=J.rangeExtent?J.rangeExtent():q(J.range());R=K.selectAll(".domain").data([0]);var C=(R.enter().append("path").attr("class","domain"),b.withoutTransition?R:a.transition(R));I.append("line");I.append("text");var B=I.select("line"),D=H.select("line"),da=I.select("text"),E=H.select("text");b.isCategory?(F=Math.ceil((M(1)-M(0))/2),R=Y?0:F,A=Y?F:0):F=R=0;var Z,S=t(K.select(".tick")),N=[],K=Math.max(6,0)+3,W="left"===h||"right"===h,w=w.select("text");Z=w.selectAll("tspan").data(function(a,e){var l=
b.tickMultiline?r(a,b.tickWidth):[].concat(g(a));N[e]=l.length;return l.map(function(a){return{index:e,splitted:a}})});Z.enter().append("tspan");Z.exit().remove();Z.text(function(a){return a.splitted});var X=b.tickTextRotate;switch(h){case "bottom":O=e;B.attr("y2",6);da.attr("y",K);D.attr("x1",R).attr("x2",R).attr("y2",ca);E.attr("x",0).attr("y",X?11.5-X/15*2.5*(0<X?1:-1):K).style("text-anchor",X?0<X?"start":"end":"middle").attr("transform",X?"rotate("+X+")":"");Z.attr("x",0).attr("dy",u).attr("dx",
X?8*Math.sin(X/180*Math.PI):0);C.attr("d","M"+Q[0]+","+m+"V0H"+Q[1]+"V"+m);break;case "top":O=e;B.attr("y2",-6);da.attr("y",-K);D.attr("x2",0).attr("y2",-6);E.attr("x",0).attr("y",-K);w.style("text-anchor","middle");Z.attr("x",0).attr("dy","0em");C.attr("d","M"+Q[0]+","+-m+"V0H"+Q[1]+"V"+-m);break;case "left":O=l;B.attr("x2",-6);da.attr("x",-K);D.attr("x2",-6).attr("y1",A).attr("y2",A);E.attr("x",-K).attr("y",F);w.style("text-anchor","end");Z.attr("x",-K).attr("dy",u);C.attr("d","M"+-m+","+Q[0]+"H0V"+
Q[1]+"H"+-m);break;case "right":O=l,B.attr("x2",6),da.attr("x",K),D.attr("x2",6).attr("y2",0),E.attr("x",K).attr("y",0),w.style("text-anchor","start"),Z.attr("x",K).attr("dy",u),C.attr("d","M"+m+","+Q[0]+"H0V"+Q[1]+"H"+m)}if(M.rangeBand)var P=M,aa=P.rangeBand()/2,G=M=function(a){return P(a)+aa};else G.rangeBand?G=M:x.call(O,M);I.call(O,G);H.call(O,M)})}var J=a.scale.linear(),h="bottom",m,n=null,r,u,F=0,G=!0,Y;b=b||{};m=b.withOuterTick?6:0;y.scale=function(a){if(!arguments.length)return J;J=a;return y};
y.orient=function(a){if(!arguments.length)return h;h=a in{top:1,right:1,bottom:1,left:1}?a+"":"bottom";return y};y.tickFormat=function(a){if(!arguments.length)return r;r=a;return y};y.tickCentered=function(a){if(!arguments.length)return Y;Y=a;return y};y.tickOffset=function(){return F};y.tickInterval=function(){var a;b.isCategory?a=2*F:(a=y.g.select("path.domain").node().getTotalLength()-2*m,a/=y.g.selectAll("line").size());return Infinity===a?0:a};y.ticks=function(){if(!arguments.length)return u;
u=arguments;return y};y.tickCulling=function(a){if(!arguments.length)return G;G=a;return y};y.tickValues=function(a){if("function"===typeof a)n=function(){return a(J.domain())};else{if(!arguments.length)return n;n=a}return y};return y}var A={version:"0.4.10"},u,f;A.generate=function(a){return new n(a)};A.chart={fn:n.prototype,internal:{fn:r.prototype,axis:{fn:m.prototype}}};u=A.chart.fn;f=A.chart.internal.fn;f.init=function(){var a=this.config;this.initParams();if(a.data_url)this.convertUrlToData(a.data_url,
...
this.hasArcType()&&this.updateTargetsForArc&&this.updateTargetsForArc(a);this.updateTargetsForSubchart&&this.updateTargetsForSubchart(a);this.showTargets()};f.showTargets=function(){var a=this;a.svg.selectAll("."+g.target).filter(function(b){return a.isTargetToShow(b.id)}).transition().duration(a.config.transition_duration).style("opacity",1)};f.redraw=function(a,b){var e=this,l=e.main,q=e.d3,f=e.config,p=e.getShapeIndices(e.isAreaType),T=e.getShapeIndices(e.isBarType),t=e.getShapeIndices(e.isLineType),
y,J,h,m,n,r,u,w,G,Y,ca,ea,U,L=e.hasArcType(),x,A,M,V,C,B,D,E,O=e.filterTargetsToShow(e.data.targets),R,ha,Q,S=e.xv.bind(e),N,P;a=a||{};y=H(a,"withY",!0);J=H(a,"withSubchart",!0);h=H(a,"withTransition",!0);r=H(a,"withTransform",!1);u=H(a,"withUpdateXDomain",!1);w=H(a,"withUpdateOrgXDomain",!1);G=H(a,"withTrimXDomain",!0);U=H(a,"withUpdateXAxis",u);Y=H(a,"withLegend",!1);ca=H(a,"withEventRect",!0);ea=H(a,"withDimension",!0);m=H(a,"withTransitionForExit",h);n=H(a,"withTransitionForAxis",h);B=h?f.transition_duration:
0;m=m?B:0;b=b||e.axis.generateTransitions(n?B:0);Y&&f.legend_show?e.updateLegend(e.mapToIds(e.data.targets),a,b):ea&&e.updateDimension(!0);e.isCategorized()&&0===O.length&&e.x.domain([0,e.axes.x.selectAll(".tick").size()]);O.length?(e.updateXDomain(O,u,w,G),f.axis_x_tick_values||(R=e.axis.updateXAxisTickValues(O))):(e.xAxis.tickValues([]),e.subXAxis.tickValues([]));f.zoom_rescale&&!a.flow&&(Q=e.x.orgDomain());e.y.domain(e.getYDomain(O,"y",Q));e.y2.domain(e.getYDomain(O,"y2",Q));!f.axis_y_tick_values&&
f.axis_y_tick_count&&e.yAxis.tickValues(e.axis.generateTickValues(e.y.domain(),f.axis_y_tick_count));!f.axis_y2_tick_values&&f.axis_y2_tick_count&&e.y2Axis.tickValues(e.axis.generateTickValues(e.y2.domain(),f.axis_y2_tick_count));e.axis.redraw(b,L);e.axis.updateLabels(h);if((u||U)&&O.length)if(f.axis_x_tick_culling&&R){for(h=1;h<R.length;h++)if(R.length/h<f.axis_x_tick_culling_max){ha=h;break}e.svg.selectAll("."+g.axisX+" .tick text").each(function(a){a=R.indexOf(a);0<=a&&q.select(this).style("display",
a%ha?"none":"block")})}else e.svg.selectAll("."+g.axisX+" .tick text").style("display","block");x=e.generateDrawArea?e.generateDrawArea(p,!1):void 0;A=e.generateDrawBar?e.generateDrawBar(T):void 0;M=e.generateDrawLine?e.generateDrawLine(t,!1):void 0;V=e.generateXYForText(p,T,t,!0);C=e.generateXYForText(p,T,t,!1);y&&(e.subY.domain(e.getYDomain(O,"y")),e.subY2.domain(e.getYDomain(O,"y2")));e.tooltip.style("display","none");e.updateXgridFocus();l.select("text."+g.text+"."+g.empty).attr("x",e.width/2).attr("y",
e.height/2).text(f.data_empty_label_text).transition().style("opacity",O.length?0:1);e.updateGrid(B);e.updateRegion(B);e.updateBar(m);e.updateLine(m);e.updateArea(m);e.updateCircle();e.hasDataLabel()&&e.updateText(m);e.redrawTitle&&e.redrawTitle();e.redrawArc&&e.redrawArc(B,m,r);e.redrawSubchart&&e.redrawSubchart(J,b,B,m,p,T,t);l.selectAll("."+g.selectedCircles).filter(e.isBarType.bind(e)).selectAll("circle").remove();f.interaction_enabled&&!a.flow&&ca&&(e.redrawEventRect(),e.updateZoom&&e.updateZoom());
e.updateCircleY();N=(e.config.axis_rotated?e.circleY:e.circleX).bind(e);P=(e.config.axis_rotated?e.circleX:e.circleY).bind(e);a.flow&&(E=e.generateFlow({targets:O,flow:a.flow,duration:a.flow.duration,drawBar:A,drawLine:M,drawArea:x,cx:N,cy:P,xv:S,xForText:V,yForText:C}));(B||E)&&e.isTabVisible()?q.transition().duration(B).each(function(){var b=[];[e.redrawBar(A,!0),e.redrawLine(M,!0),e.redrawArea(x,!0),e.redrawCircle(N,P,!0),e.redrawText(V,C,a.flow,!0),e.redrawRegion(!0),e.redrawGrid(!0)].forEach(function(a){a.forEach(function(a){b.push(a)})});
D=e.generateWait();b.forEach(function(a){D.add(a)})}).call(D,function(){E&&E();f.onrendered&&f.onrendered.call(e)}):(e.redrawBar(A),e.redrawLine(M),e.redrawArea(x),e.redrawCircle(N,P),e.redrawText(V,C,a.flow),e.redrawRegion(),e.redrawGrid(),f.onrendered&&f.onrendered.call(e));e.mapToIds(e.data.targets).forEach(function(a){e.withoutFadeIn[a]=!0})};f.updateAndRedraw=function(a){var b=this.config,e;a=a||{};a.withTransition=H(a,"withTransition",!0);a.withTransform=H(a,"withTransform",!1);a.withLegend=
...
withTransitionForAxis:!1})};f.isTimeSeries=function(){return"timeseries"===this.config.axis_x_type};f.isCategorized=function(){return 0<=this.config.axis_x_type.indexOf("categor")};f.isCustomX=function(){var a=this.config;return!this.isTimeSeries()&&(a.data_x||D(a.data_xs))};f.isTimeSeriesY=function(){return"timeseries"===this.config.axis_y_type};f.getTranslate=function(a){var b=this.config,e,l;"main"===a?(e=W(this.margin.left),l=W(this.margin.top)):"context"===a?(e=W(this.margin2.left),l=W(this.margin2.top)):
...
this.additionalConfig[b]},this);return a};f.additionalConfig={};f.loadConfig=function(a){function b(){var a=q.shift();return a&&l&&"object"===typeof l&&a in l?(l=l[a],b()):a?void 0:l}var e=this.config,l,q,f;Object.keys(e).forEach(function(p){l=a;q=p.split("_");f=b();B(f)&&(e[p]=f)})};f.getScale=function(a,b,e){return(e?this.d3.time.scale():this.d3.scale.linear()).range([a,b])};f.getX=function(a,b,e,l){var q=this.getScale(a,b,this.isTimeSeries()),f=e?q.domain(e):q,p;this.isCategorized()?(l=l||function(){return 0},
...
t=w(p)?p:w(g)?t<g?t:g-10:t;a=w(g)?g:w(p)?p<a?a:p+10:a;if(0===f.length)return"y2"===b?l.y2.domain():l.y.domain();isNaN(t)&&(t=0);isNaN(a)&&(a=t);t===a&&(0>t?a=0:t=0);f=0<=t&&0<=a;n=0>=t&&0>=a;if(w(p)&&f||w(g)&&n)r=!1;r&&(f&&(t=0),n&&(a=0));p=Math.abs(a-t);g=y=.1*p;"undefined"!==typeof h&&(t=Math.max(Math.abs(t),Math.abs(a)),a=h+t,t=h-t);m?(h=l.getDataLabelLength(t,a,"width"),m=S(l.y.range()),h=[h[0]/m,h[1]/m],g+=h[1]/(1-h[0]-h[1])*p,y+=h[0]/(1-h[0]-h[1])*p):u&&(h=l.getDataLabelLength(t,a,"height"),
g+=l.axis.convertPixelsToAxisPadding(h[1],p),y+=l.axis.convertPixelsToAxisPadding(h[0],p));"y"===b&&D(q.axis_y_padding)&&(g=l.axis.getPadding(q.axis_y_padding,"top",g,p),y=l.axis.getPadding(q.axis_y_padding,"bottom",y,p));"y2"===b&&D(q.axis_y2_padding)&&(g=l.axis.getPadding(q.axis_y2_padding,"top",g,p),y=l.axis.getPadding(q.axis_y2_padding,"bottom",y,p));r&&(f&&(y=t),n&&(g=-a));q=[t-y,a+g];return e?q.reverse():q};f.getXDomainMin=function(a){var b=this,e=b.config;return B(e.axis_x_min)?b.isTimeSeries()?
this.parseDate(e.axis_x_min):e.axis_x_min:b.d3.min(a,function(a){return b.d3.min(a.values,function(a){return a.x})})};f.getXDomainMax=function(a){var b=this,e=b.config;return B(e.axis_x_max)?b.isTimeSeries()?this.parseDate(e.axis_x_max):e.axis_x_max:b.d3.max(a,function(a){return b.d3.max(a.values,function(a){return a.x})})};f.getXDomainPadding=function(a){var b=this.config;a=a[1]-a[0];var e,l;this.isCategorized()?a=0:this.hasType("bar")?(e=this.getMaxDataCount(),a=1<e?a/(e-1)/2:.5):a*=.01;"object"===
typeof b.axis_x_padding&&D(b.axis_x_padding)?(e=w(b.axis_x_padding.left)?b.axis_x_padding.left:a,l=w(b.axis_x_padding.right)?b.axis_x_padding.right:a):e="number"===typeof b.axis_x_padding?l=b.axis_x_padding:l=a;return{left:e,right:l}};f.getXDomain=function(a){var b=[this.getXDomainMin(a),this.getXDomainMax(a)];a=b[0];var e=b[1],b=this.getXDomainPadding(b),l=0,q=0;0!==a-e||this.isCategorized()||(this.isTimeSeries()?(a=new Date(.5*a.getTime()),e=new Date(1.5*e.getTime())):(a=0===a?1:.5*a,e=0===e?-1:
...
this.config;return"string"===typeof a.data_order&&"desc"===a.data_order.toLowerCase()};f.isOrderAsc=function(){var a=this.config;return"string"===typeof a.data_order&&"asc"===a.data_order.toLowerCase()};f.orderTargets=function(a){var b=this.config,e=this.isOrderAsc(),l=this.isOrderDesc();e||l?a.sort(function(a,b){var l=function(a,b){return a+Math.abs(b.value)},f=a.values.reduce(l,0),l=b.values.reduce(l,0);return e?l-f:f-l}):N(b.data_order)&&a.sort(b.data_order);return a};f.filterByX=function(a,b){return this.d3.merge(a.map(function(a){return a.values})).filter(function(a){return 0===
a.x-b})};f.filterRemoveNull=function(a){return a.filter(function(a){return w(a.value)})};f.filterByXDomain=function(a,b){return a.map(function(a){return{id:a.id,id_org:a.id_org,values:a.values.filter(function(a){return b[0]<=a.x&&a.x<=b[1]})}})};f.hasDataLabel=function(){var a=this.config;return"boolean"===typeof a.data_labels&&a.data_labels||"object"===typeof a.data_labels&&D(a.data_labels)?!0:!1};f.getDataLabelLength=function(a,b,e){var l=this,q=[0,0];l.selectChart.select("svg").selectAll(".dummy").data([a,
...
function(a){return this.convertXsvToData(a,this.d3.csv)};f.convertTsvToData=function(a){return this.convertXsvToData(a,this.d3.tsv)};f.convertJsonToData=function(a,b){var e=[],l,f;b?(b.x?(l=b.value.concat(b.x),this.config.data_x=b.x):l=b.value,e.push(l),a.forEach(function(a){var b=[];l.forEach(function(e){e=E(a[e])?null:a[e];b.push(e)});e.push(b)}),f=this.convertRowsToData(e)):(Object.keys(a).forEach(function(b){e.push([b].concat(a[b]))}),f=this.convertColumnsToData(e));return f};f.convertRowsToData=
function(a){var b=a[0],e={},l=[],f,g;for(f=1;f<a.length;f++){e={};for(g=0;g<a[f].length;g++){if(E(a[f][g]))throw Error("Source data is missing a component at ("+f+","+g+")!");e[b[g]]=a[f][g]}l.push(e)}return l};f.convertColumnsToData=function(a){var b=[],e,l,f;for(e=0;e<a.length;e++)for(f=a[e][0],l=1;l<a[e].length;l++){E(b[l-1])&&(b[l-1]={});if(E(a[e][l]))throw Error("Source data is missing a component at ("+e+","+l+")!");b[l-1][f]=a[e][l]}return b};f.convertDataToTargets=function(a,b){var e=this,
l=e.config,f=e.d3.keys(a[0]).filter(e.isNotX,e),g=e.d3.keys(a[0]).filter(e.isX,e);f.forEach(function(f){var q=e.getXKey(f);e.isCustomX()||e.isTimeSeries()?0<=g.indexOf(q)?e.data.xs[f]=(b&&e.data.xs[f]?e.data.xs[f]:[]).concat(a.map(function(a){return a[q]}).filter(w).map(function(a,b){return e.generateTargetX(a,f,b)})):l.data_x?e.data.xs[f]=e.getOtherTargetXs():D(l.data_xs)&&(e.data.xs[f]=e.getXValuesOfXKey(q,e.data.targets)):e.data.xs[f]=a.map(function(a,b){return b})});f.forEach(function(a){if(!e.data.xs[a])throw Error('x is not defined for id = "'+
a+'".');});f=f.map(function(b,f){var q=l.data_idConverter(b);return{id:q,id_org:b,values:a.map(function(a,g){var v=e.getXKey(b),v=a[v],h=e.generateTargetX(v,b,g);e.isCustomX()&&e.isCategorized()&&0===f&&v&&(0===g&&(l.axis_x_categories=[]),l.axis_x_categories.push(v));if(E(a[b])||e.data.xs[b].length<=g)h=void 0;return{x:h,value:null===a[b]||isNaN(a[b])?null:+a[b],id:q}}).filter(function(a){return B(a.x)})}});f.forEach(function(a){var b;l.data_xSort&&(a.values=a.values.sort(function(a,b){return(a.x||
...
0;e.isLineType(a)?l.data_regions[a.id]?a=e.lineWithRegions(g,v,h,l.data_regions[a.id]):(e.isStepType(a)&&(g=e.convertValuesToStep(g)),a=f.interpolate(e.getInterpolate(a))(g)):(g[0]&&(t=v(g[0].x),T=h(g[0].value)),a=l.axis_rotated?"M "+T+" "+t:"M "+t+" "+T);return a?a:"M 0 0"}};f.generateGetLinePoints=function(a,b){var e=this,l=e.config,f=e.getShapeX(0,a.__max__+1,a,!!b),g=e.getShapeY(!!b),p=e.getShapeOffset(e.isLineType,a,!!b),h=b?e.getSubYScale:e.getYScale;return function(a,b){var m=h.call(e,a.id)(0),
n=p(a,b)||m,r=f(a),u=g(a);l.axis_rotated&&(0<a.value&&u<m||0>a.value&&m<u)&&(u=m);return[[r,u-(m-n)],[r,u-(m-n)],[r,u-(m-n)],[r,u-(m-n)]]}};f.lineWithRegions=function(a,b,e,l){function f(a,b){var e;for(e=0;e<b.length;e++)if(b[e].start<a&&a<=b[e].end)return!0;return!1}function g(a){return"M"+a[0][0]+" "+a[0][1]+" "+a[1][0]+" "+a[1][1]}var p=this.config,h,t,m="M",J,n,r,u,w=this.isCategorized()?.5:0,I,F,G=[];if(B(l))for(h=0;h<l.length;h++)G[h]={},E(l[h].start)?G[h].start=a[0].x:G[h].start=this.isTimeSeries()?
this.parseDate(l[h].start):l[h].start,E(l[h].end)?G[h].end=a[a.length-1].x:G[h].end=this.isTimeSeries()?this.parseDate(l[h].end):l[h].end;I=p.axis_rotated?function(a){return e(a.value)}:function(a){return b(a.x)};F=p.axis_rotated?function(a){return b(a.x)}:function(a){return e(a.value)};l=this.isTimeSeries()?function(a,l,f,q){var h=a.x.getTime();l=l.x-a.x;a=new Date(h+l*f);h=new Date(h+l*(f+q));f=p.axis_rotated?[[e(n(f)),b(a)],[e(n(f+q)),b(h)]]:[[b(a),e(n(f))],[b(h),e(n(f+q))]];return g(f)}:function(a,
l,f,q){a=p.axis_rotated?[[e(n(f),!0),b(J(f))],[e(n(f+q),!0),b(J(f+q))]]:[[b(J(f),!0),e(n(f))],[b(J(f+q),!0),e(n(f+q))]];return g(a)};for(h=0;h<a.length;h++)if(E(G)||!f(a[h].x,G))m+=" "+I(a[h])+" "+F(a[h]);else for(J=this.getScale(a[h-1].x+w,a[h].x+w,this.isTimeSeries()),n=this.getScale(a[h-1].value,a[h].value),t=b(a[h].x)-b(a[h-1].x),r=e(a[h].value)-e(a[h-1].value),t=Math.sqrt(Math.pow(t,2)+Math.pow(r,2)),r=2/t,u=2*r,t=r;1>=t;t+=u)m+=l(a[h-1],a[h],t,r);return m};f.updateArea=function(a){var b=this,
...
v.getTextRect(a.textContent,g.legendItem));return v.legendItemTextBox[b]}function h(b,e,g){function q(a,b){b||(r=(n-F-m)/2,10>r&&(r=(n-m)/2,F=0,L++));U[a]=L;D[L]=v.isLegendInset?10:r;G[a]=F;F+=m}var t=0===g;g=g===a.length-1;b=f(b,e);g=b.width+15+(!g||v.isLegendRight||v.isLegendInset?10:0);b=b.height+4;var m=v.isLegendRight||v.isLegendInset?b:g,n=v.isLegendRight||v.isLegendInset?v.getLegendHeight():v.getLegendWidth(),r;t&&(K=x=L=F=0);if(p.legend_show&&!v.isLegendToShow(e))A[e]=C[e]=U[e]=G[e]=0;else{A[e]=
g;C[e]=b;if(!x||g>=x)x=g;if(!K||b>=K)K=b;t=v.isLegendRight||v.isLegendInset?K:x;p.legend_equally?(Object.keys(A).forEach(function(a){A[a]=x}),Object.keys(C).forEach(function(a){C[a]=K}),r=(n-t*a.length)/2,10>r?(L=F=0,a.forEach(function(a){q(a)})):q(e,!0)):q(e)}}var v=this,p=v.config,m,t,n,r,u,w,x=0,K=0,I,F=0,G={},A={},C={},D=[0],U={},L=0,E,N;b=b||{};E=H(b,"withTransition",!0);b=H(b,"withTransitionForTransform",!0);v.isLegendInset&&(L=p.legend_inset_step?p.legend_inset_step:a.length,v.updateLegendStep(L));
v.isLegendRight?(m=function(a){return x*U[a]},r=function(a){return D[U[a]]+G[a]}):v.isLegendInset?(m=function(a){return x*U[a]+10},r=function(a){return D[U[a]]+G[a]}):(m=function(a){return D[U[a]]+G[a]},r=function(a){return K*U[a]});t=function(a,b){return m(a,b)+14};u=function(a,b){return r(a,b)+9};n=function(a,b){return m(a,b)};w=function(a,b){return r(a,b)-5};I=v.legend.selectAll("."+g.legendItem).data(a).enter().append("g").attr("class",function(a){return v.generateClass(g.legendItem,a)}).style("visibility",
...
!1);v.api.revert();p.legend_item_onmouseout&&p.legend_item_onmouseout.call(v,a)});I.append("text").text(function(a){return B(p.data_names[a])?p.data_names[a]:a}).each(function(a,b){h(this,a,b)}).style("pointer-events","none").attr("x",v.isLegendRight||v.isLegendInset?t:-200).attr("y",v.isLegendRight||v.isLegendInset?-200:u);I.append("rect").attr("class",g.legendItemEvent).style("fill-opacity",0).attr("x",v.isLegendRight||v.isLegendInset?n:-200).attr("y",v.isLegendRight||v.isLegendInset?-200:w);I.append("rect").attr("class",
g.legendItemTile).style("pointer-events","none").style("fill",v.color).attr("x",v.isLegendRight||v.isLegendInset?t:-200).attr("y",v.isLegendRight||v.isLegendInset?-200:r).attr("width",10).attr("height",10);I=v.legend.select("."+g.legendBackground+" rect");v.isLegendInset&&0<x&&0===I.size()&&(I=v.legend.insert("g","."+g.legendItem).attr("class",g.legendBackground).append("rect"));N=v.legend.selectAll("text").data(a).text(function(a){return B(p.data_names[a])?p.data_names[a]:a}).each(function(a,b){h(this,
a,b)});(E?N.transition():N).attr("x",t).attr("y",u);t=v.legend.selectAll("rect."+g.legendItemEvent).data(a);(E?t.transition():t).attr("width",function(a){return A[a]}).attr("height",function(a){return C[a]}).attr("x",n).attr("y",w);n=v.legend.selectAll("rect."+g.legendItemTile).data(a);(E?n.transition():n).style("fill",v.color).attr("x",m).attr("y",r);I&&(E?I.transition():I).attr("height",v.getLegendHeight()-12).attr("width",x*(L+1)+10);v.legend.selectAll("."+g.legendItem).classed(g.legendItemHidden,
function(a){return!v.isTargetToShow(a)});v.updateLegendItemWidth(x);v.updateLegendItemHeight(K);v.updateLegendStep(L);v.updateSizes();v.updateScales();v.updateSvgSize();v.transformAll(b,e);v.legendHasRendered=!0};f.initTitle=function(){this.title=this.svg.append("text").text(this.config.title_text).attr("class",this.CLASS.title)};f.redrawTitle=function(){this.title.attr("x",this.xForTitle.bind(this)).attr("y",this.yForTitle.bind(this))};f.xForTitle=function(){var a=this.config,b=a.title_position||
...
(N(b.axis_x_tick_format)?e=b.axis_x_tick_format:a.isTimeSeries()&&(e=function(e){return e?a.axisTimeFormat(b.axis_x_tick_format)(e):""}));return N(e)?function(b){return e.call(a,b)}:e};m.prototype.getTickValues=function(a,b){return a?a:b?b.tickValues():void 0};m.prototype.getXAxisTickValues=function(){return this.getTickValues(this.owner.config.axis_x_tick_values,this.owner.xAxis)};m.prototype.getYAxisTickValues=function(){return this.getTickValues(this.owner.config.axis_y_tick_values,this.owner.yAxis)};
...
b).attr("x",this.xForY2AxisLabel.bind(this)).attr("dx",this.dxForY2AxisLabel.bind(this)).attr("dy",this.dyForY2AxisLabel.bind(this)).text(this.textForY2AxisLabel.bind(this))};m.prototype.getPadding=function(a,b,e,f){return w(a[b])?"ratio"===a.unit?a[b]*f:this.convertPixelsToAxisPadding(a[b],f):e};m.prototype.convertPixelsToAxisPadding=function(a,b){var e=this.owner;return a/(e.config.axis_rotated?e.width:e.height)*b};m.prototype.generateTickValues=function(a,b,e){var f=a,g,h,p,m;if(b)if(b=N(b)?b():
...
g.shape).filter(function(a){return e.data_selection_isselectable(a)}).each(function(a,e){var f=h.select(this),l=f.classed(g.SELECTED),m=f.classed(g.INCLUDED),p,n,v,T,x;p=!1;if(f.classed(g.circle))p=1*f.attr("cx"),n=1*f.attr("cy"),x=b.togglePoint,p=t<p&&p<r&&u<n&&n<w;else if(f.classed(g.bar))x=ja(this),p=x.x,n=x.y,v=x.width,T=x.height,x=b.togglePath,p=!(r<p||p+v<t)&&!(w<n||n+T<u);else return;p^m&&(f.classed(g.INCLUDED,!m),f.classed(g.SELECTED,!l),x.call(b,!l,f,a,e))}))};f.dragstart=function(a){var b=
...
this;return a&&a.length?a.map(function(a){return b.selectorLegend(a)}):null};var w=f.isValue=function(a){return a||0===a},N=f.isFunction=function(a){return"function"===typeof a},C=f.isString=function(a){return"string"===typeof a},E=f.isUndefined=function(a){return"undefined"===typeof a},B=f.isDefined=function(a){return"undefined"!==typeof a},aa=f.ceil10=function(a){return 10*Math.ceil(a/10)},W=f.asHalfPixel=function(a){return Math.ceil(a)+.5},S=f.diffDomain=function(a){return a[1]-a[0]},ba=f.isEmpty=
function(a){return!a||C(a)&&0===a.length||"object"===typeof a&&0===Object.keys(a).length},D=f.notEmpty=function(a){return 0<Object.keys(a).length},H=f.getOption=function(a,b,e){return B(a[b])?a[b]:e},ia=f.hasValue=function(a,b){var e=!1;Object.keys(a).forEach(function(f){a[f]===b&&(e=!0)});return e},ja=f.getPathBox=function(a){var b=a.getBoundingClientRect(),e=[a.pathSegList.getItem(0),a.pathSegList.getItem(1)];a=e[0].x;e=Math.min(e[0].y,e[1].y);return{x:a,y:e,width:b.width,height:b.height}};u.focus=
...
a||{};a instanceof Array?a={ids:a}:"string"===typeof a&&(a={ids:[a]});b.unload(b.mapToTargetIds(a.ids),function(){b.redraw({withUpdateOrgXDomain:!0,withUpdateXDomain:!0,withLegend:!0});a.done&&a.done()})};u.flow=function(a){var b=this.internal,e,f,g=[],h=b.getMaxDataCount(),m,n=0,r=0,u;if(a.json)f=b.convertJsonToData(a.json,a.keys);else if(a.rows)f=b.convertRowsToData(a.rows);else if(a.columns)f=b.convertColumnsToData(a.columns);else return;e=b.convertDataToTargets(f,!0);b.data.targets.forEach(function(a){var f=
!1,l,h;for(l=0;l<e.length;l++)if(a.id===e[l].id){f=!0;a.values[a.values.length-1]&&(r=a.values[a.values.length-1].index+1);n=e[l].values.length;for(h=0;h<n;h++)e[l].values[h].index=r+h,b.isTimeSeries()||(e[l].values[h].x=r+h);a.values=a.values.concat(e[l].values);e.splice(l,1);break}f||g.push(a.id)});b.data.targets.forEach(function(a){var e,f;for(e=0;e<g.length;e++)if(a.id===g[e])for(r=a.values[a.values.length-1].index+1,f=0;f<n;f++)a.values.push({id:a.id,index:r+f,x:b.isTimeSeries()?b.getOtherTargetX(r+
f):r+f,value:null})});b.data.targets.length&&e.forEach(function(a){var e,f=[];for(e=b.data.targets[0].values[0].index;e<r;e++)f.push({id:a.id,index:e,x:b.isTimeSeries()?b.getOtherTargetX(e):e,value:null});a.values.forEach(function(a){a.index+=r;b.isTimeSeries()||(a.x+=r)});a.values=f.concat(a.values)});b.data.targets=b.data.targets.concat(e);b.getMaxDataCount();m=b.data.targets[0];f=m.values[0];B(a.to)?(n=0,u=b.isTimeSeries()?b.parseDate(a.to):a.to,m.values.forEach(function(a){a.x<u&&n++})):B(a.length)&&
...
h,withTrimXDomain:!1,withUpdateXAxis:!0})};f.generateFlow=function(a){var b=this,e=b.config,f=b.d3;return function(){var h=a.targets,m=a.flow,n=a.drawBar,r=a.drawLine,t=a.drawArea,u=a.cx,w=a.cy,x=a.xv,A=a.xForText,C=a.yForText,B=a.duration,I=1,F,G=m.index,D=m.length,E=b.getValueOnIndex(b.data.targets[0].values,G),H=b.getValueOnIndex(b.data.targets[0].values,G+D),I=b.x.domain(),B=m.duration||B,N=m.done||function(){},L=b.generateWait(),P=b.xgrid||f.selectAll([]),W=b.xgridLines||f.selectAll([]),M=b.mainRegion||
f.selectAll([]),V=b.mainText||f.selectAll([]),aa=b.mainBar||f.selectAll([]),ba=b.mainLine||f.selectAll([]),fa=b.mainArea||f.selectAll([]),ga=b.mainCircle||f.selectAll([]);b.flowing=!0;b.data.targets.forEach(function(a){a.values.splice(0,D)});h=b.updateXDomain(h,!0,!0);b.updateXGrid&&b.updateXGrid(!0);m.orgDataCount?m=1===m.orgDataCount||E.x===H.x?b.x(I[0])-b.x(h[0]):b.isTimeSeries()?b.x(I[0])-b.x(h[0]):b.x(E.x)-b.x(H.x):1!==b.data.targets[0].values.length?m=b.x(I[0])-b.x(h[0]):b.isTimeSeries()?(E=
b.getValueOnIndex(b.data.targets[0].values,0),H=b.getValueOnIndex(b.data.targets[0].values,b.data.targets[0].values.length-1),m=b.x(E.x)-b.x(H.x)):m=S(h)/2;I=S(I)/S(h);F="translate("+m+",0) scale("+I+",1)";b.hideXGridFocus();b.hideTooltip();f.transition().ease("linear").duration(B).each(function(){L.add(b.axes.x.transition().call(b.xAxis));L.add(aa.transition().attr("transform",F));L.add(ba.transition().attr("transform",F));L.add(fa.transition().attr("transform",F));L.add(ga.transition().attr("transform",
F));L.add(V.transition().attr("transform",F));L.add(M.filter(b.isRegionOnX).transition().attr("transform",F));L.add(P.transition().attr("transform",F));L.add(W.transition().attr("transform",F))}).call(L,function(){var a,f=[],l=[],h=[];if(D){for(a=0;a<D;a++)f.push("."+g.shape+"-"+(G+a)),l.push("."+g.text+"-"+(G+a)),h.push("."+g.eventRect+"-"+(G+a));b.svg.selectAll("."+g.shapes).selectAll(f).remove();b.svg.selectAll("."+g.texts).selectAll(l).remove();b.svg.selectAll("."+g.eventRects).selectAll(h).remove();
b.svg.select("."+g.xgrid).remove()}P.attr("transform",null).attr(b.xgridAttr);W.attr("transform",null);W.select("line").attr("x1",e.axis_rotated?0:x).attr("x2",e.axis_rotated?b.width:x);W.select("text").attr("x",e.axis_rotated?b.width:0).attr("y",x);aa.attr("transform",null).attr("d",n);ba.attr("transform",null).attr("d",r);fa.attr("transform",null).attr("d",t);ga.attr("transform",null).attr("cx",u).attr("cy",w);V.attr("transform",null).attr("x",A).attr("y",C).style("fill-opacity",b.opacityForText.bind(b));
M.attr("transform",null);M.select("rect").filter(b.isRegionOnX).attr("x",b.regionX.bind(b)).attr("width",b.regionWidth.bind(b));e.interaction_enabled&&b.redrawEventRect();N();b.flowing=!1})}};u.selected=function(a){var b=this.internal,e=b.d3;return e.merge(b.main.selectAll("."+g.shapes+b.getTargetSelectorSuffix(a)).selectAll("."+g.shape).filter(function(){return e.select(this).classed(g.SELECTED)}).map(function(a){return a.map(function(a){a=a.__data__;return a.data?a.data:a})}))};u.select=function(a,
b,e){var f=this.internal,h=f.d3,m=f.config;m.data_selection_enabled&&f.main.selectAll("."+g.shapes).selectAll("."+g.shape).each(function(n,r){var t=h.select(this),u=n.data?n.data.id:n.id,w=f.getToggle(this,n).bind(f),u=m.data_selection_grouped||!a||0<=a.indexOf(u),x=!b||0<=b.indexOf(r),A=t.classed(g.SELECTED);t.classed(g.line)||t.classed(g.area)||(u&&x?m.data_selection_isselectable(n)&&!A&&w(!0,t.classed(g.SELECTED,!0),n,r):B(e)&&e&&A&&w(!1,t.classed(g.SELECTED,!1),n,r))})};u.unselect=function(a,
...
typeof a?(w(a.x)&&(e.axis_x_min=a.x),w(a.y)&&(e.axis_y_min=a.y),w(a.y2)&&(e.axis_y2_min=a.y2)):e.axis_y_min=e.axis_y2_min=a,b.redraw({withUpdateOrgXDomain:!0,withUpdateXDomain:!0});else return{x:e.axis_x_min,y:e.axis_y_min,y2:e.axis_y2_min}};u.axis.range=function(a){if(arguments.length)B(a.max)&&this.axis.max(a.max),B(a.min)&&this.axis.min(a.min);else return{max:this.axis.max(),min:this.axis.min()}};u.legend=function(){};u.legend.show=function(a){var b=this.internal;b.showLegend(b.mapToTargetIds(a));
...
0)};var P;f.isSafari=function(){var a=h.navigator.userAgent;return 0<=a.indexOf("Safari")&&0>a.indexOf("Chrome")};f.isChrome=function(){return 0<=h.navigator.userAgent.indexOf("Chrome")};Function.prototype.bind||(Function.prototype.bind=function(a){if("function"!==typeof this)throw new TypeError("Function.prototype.bind - what is trying to be bound is not callable");var b=Array.prototype.slice.call(arguments,1),e=this,f=function(){},g=function(){return e.apply(this instanceof f?this:a,b.concat(Array.prototype.slice.call(arguments)))};
f.prototype=this.prototype;g.prototype=new f;return g});"function"===typeof define&&define.amd?define("c3",["d3"],A):"undefined"!==typeof exports&&"undefined"!==typeof module?module.exports=A:h.c3=A})(window);function doBreakdown(h,n){c3.generate({data:{columns:[["Drops"].concat(h)],type:"bar"},bindto:"#tabs-3",tooltip:{contents:function(h,m,x,A){if(n){m="";m="<div id='drops_tooltip'>"+h[0].value+" drop(s) at "+h[0].index+" mana.<br>";for(var u in n[h[0].index])for(A=n[h[0].index][u],x=Object.keys(A)[0],A=A[x],i=0;i<A;i++)m=m+'<img width="100" src="'+getCardFileSmall(x)+'"/>';return m+"</div>"}}}}).axis.labels({y:"Number of Cards",x:"Turn Played"});$(function(){var h=$("#tabs").tabs();h.find(".ui-tabs-nav").sortable({axis:"x",
stop:function(){h.tabs("refresh")}})})};function preload(h){$(h).each(function(){$("<img/>")[0].src=this})}function updateManaCostChosenCards(){rows=$("#cards_chosen tr");for(i=0;i<rows.length;i++)jrows=jQuery(rows[i]),text=jrows.find("[id=card_name]").text(),"undefined"!==typeof card_data[text]&&(jrows.prepend($('<td id="cost">'+card_data[text].cost+"</td>")),$("#no_cards").remove());0==rows.length&&$('<span id="no_cards"><i>no cards chosen yet</i></span>').appendTo($("#cards_chosen"));sortTable()}function updateBreakdown(){}
function sortTable(){var h=document.getElementById("cards_chosen").tBodies[0];if(null!=h){for(var n=[],r=0,m=h.rows.length;r<m;r++){var x=h.rows[r],A=parseFloat(x.cells[0].textContent||x.cells[0].innerText);isNaN(A)||n.push([A,x])}n.sort(function(h,f){return h[0]-f[0]});r=0;for(m=n.length;r<m;r++)h.appendChild(n[r][1]);n=null}};var selected=[null,null,null],userList=null,dat={},img="/images/cards_medium/",img_small="/images/cards_small/",card_back="/images/card_selection_back.png",rarity="none",number_element,old_index=0,selected_index=0,selected_card=0,name_to_id=[],mode="start",d=new Date,arena_id="unknown",pathArray=window.location.pathname.split("/",-1),arena_id=pathArray[3];function createElement(h,n,r){var m=$("<div/>");m.attr({id:n});m.css(r);h.append(m);return m}
function createInputButton(h,n,r,m,x,A){div=$("<div/>");div.attr({id:m,"class":m});div.html();div.css(n);h.prepend(div);h=$('<a href="#">'+r+"</a>").prependTo(div);h.click({id:x,name:m},A);return h}$(document).keydown(function(h){switch(h.which){case 13:for(z=0;3>z;z++)if(null==selected[z]){showClassCards(z);break}}});
...
function getCardFileSmall(h){return img_small+card_ids[h]+".png"}function addWaiting(h){h=getCardElement(h);var n=$('<div class="waiting"/>');n.text("Waiting for all cards to be picked...");n.appendTo(h);n.addClass(class_name)}
