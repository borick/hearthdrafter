function addDropsGraph(a){var b=[1,2,3];a&&(b=a);var e=d3.scale.ordinal().domain(d3.range(b.length)).rangeRoundBands([0,600],.05),f=d3.scale.linear().domain([0,d3.max(b)]).range([0,250]),g=d3.select("#tabs-3").append("svg").attr("width",600).attr("height",250);g.selectAll("rect").data(b).enter().append("rect").attr("x",function(a,b){return e(b)}).attr("y",function(a){return 250-f(a)}).attr("width",e.rangeBand()).attr("height",function(a){return f(a)}).attr("fill","#009DE5").on("mouseover",function(a){var b=
parseFloat(d3.select(this).attr("x"))+e.rangeBand()/2,f=parseFloat(d3.select(this).attr("y"))/2+125;d3.select("#drops_tooltip").style("left",b+"px").style("top",f+"px").select("#value").text(a);d3.select("#drops_tooltip").classed("hidden",!1)}).on("mouseout",function(){d3.select("#drops_tooltip").classed("hidden",!0)}).on("click",function(){l()});var h=!1,l=function(){h=!h;g.selectAll("rect").sort(function(a,b){return h?d3.ascending(a,b):d3.descending(a,b)}).transition().delay(function(a,b){return 50*
b}).duration(1E3).attr("x",function(a,b){return e(b)})}};function preload(a){$(a).each(function(){$("<img/>")[0].src=this})}function updateManaCostChosenCards(){rows=$("#cards_chosen tr");for(i=0;i<rows.length;i++)jrows=jQuery(rows[i]),text=jrows.find("[id=card_name]").text(),"undefined"!==typeof card_data[text]&&(jrows.prepend($('<td id="cost">'+card_data[text].cost+"</td>")),$("#no_cards").remove());0==rows.length&&$('<span id="no_cards"><i>no cards chosen yet</i></span>').appendTo($("#cards_chosen"));sortTable()}function updateBreakdown(){}
function sortTable(){var a=document.getElementById("cards_chosen").tBodies[0];if(null!=a){for(var b=[],e=0,f=a.rows.length;e<f;e++){var g=a.rows[e],h=parseFloat(g.cells[0].textContent||g.cells[0].innerText);isNaN(h)||b.push([h,g])}b.sort(function(a,b){return a[0]-b[0]});e=0;for(f=b.length;e<f;e++)a.appendChild(b[e][1]);b=null}};var selected=[null,null,null],userList=null,dat={},img="/images/cards_medium/",img_small="/images/cards_small/",card_back="/images/card_selection_back.png",rarity="none",number_element,old_index=0,selected_index=0,selected_card=0,name_to_id=[],mode="start",d=new Date,arena_id="unknown",pathArray=window.location.pathname.split("/",-1),arena_id=pathArray[3];function createElement(a,b,e){var f=$("<div/>");f.attr({id:b});f.css(e);a.append(f);return f}
...
e)}function confirmCardByName(a,b){for(i=0;i<selected.length;i++)if(selected[i]==a){confirmCard(i,b);break}}function finishConfirm(a){card_number+=1;30<=card_number?($('[class^="card"]').hide(),document.location.href="/draft/view_completed_run/"+arena_id):(updateChosenCardsTab(a),removeConfirmChoices(),updateNumber(card_number),initCardClicks(),selected=[],rarity="none",removeUndo(),removeHighlight(),$("#message_panel").hide())}
function undoLastCard(){if(0!=card_number&&window.confirm("Are you sure you want to undo the last card choice? There is no redo.")){var a="/draft/arena_action/undo_last_card_"+arena_id,b=b||[];b.push(["_trackPageview",a]);$.get(a,function(a){0!=card_number&&(--card_number,removeConfirmChoices(),removeElement(".waiting"),updateNumber(card_number),initCardClicks(),selected=[],removeUndo(),removeHighlight(),updateChosenCardsTab(a))})}}
function confirmCard(a,b){if(b)finishConfirm(b);else{var e="/draft/confirm_card_choice/"+selected[a]+"/"+arena_id,f=f||[];f.push(["_trackPageview",e]);$.get(e,function(a){finishConfirm(a)})}}function confirmCardChoice(a){a.preventDefault();a.stopPropagation();confirmCard(a.data.id)}
function updateChosenCardsTab(a){var b=Object.keys(a),e,f=b.length;b.sort();$("#cards_chosen").html("");for(e=0;e<f;e++){k=b[e];var g=$('<tr><td id="card_name"><span class="capital">'+k+'</span></td><td id="card_count">'+a[k]+"</td></tr>");$("#cards_chosen").append(g)}updateManaCostChosenCards()}function createSynergiesDiv(a){var b=$('<div id="synergies'+a+'" class="synergy_list">');a=$('<div class="outer-syn" id="outer-synergies'+a+'"><h3>Synergies</h3></div>');b.appendTo(a);return a}
