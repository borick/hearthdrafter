...
sub find_synergies {
    ...
    while (my $c = $iter->next) {
        ...
        if (_has_tag($tags_x, 'gives_taunt', $card_y) && $name_y eq 'ancient watcher') {            
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Give Ancient Watcher taunt in order to make it useful.';
        ...
        if ((_has_tag($tags_x, 'growth', $card_y) && ((_has_tag($tags_y, 'has_taunt', $card_x) || _has_tag($tags_y, 'gives_taunt', $card_x))))) {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Use taunt to protect minions that can increase in stats over time.';   
        }
        ...
        if (_has_tag($tags_x, 'panda', $card_y) && _has_tag($tags_y, 'has_charge', $card_x)) {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Return these cards to your hand for double the charge-damage.';   
        }

        # anima golem + stealth
        if (_has_tag($tags_x, 'has_stealth', $card_y) && $name_y eq 'anima golem') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Stealth cards help keep Anima Golem alive.';   
        }
        
        #demon synergy
        if (_has_tag($tags_x, 'demon_synergy', $card_y) && $race_y eq 'demon') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Certain cards have a special synergy with demons.';   
        }
        
        #mech synergy
        if (_has_tag($tags_x, 'mech_synergy', $card_y) && $race_y eq 'mech') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Certain cards have a special synergy with mechs.';   
        }        

        #pirate synergy
        if (_has_tag($tags_x, 'pirate_synergy', $card_y) && $race_y eq 'pirate') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Certain cards have a special synergy with pirates.';   
        }
        
        # lightspawn + priest/dbl health (divine spirit, inner fire
        if ($name_x eq 'divine spirit' && $name_y eq 'lightspawn') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Make lightspawn a 10/10.';   
        }
        
        # priest dbl hearth + make attack = health
        if ($name_x eq 'divine spirit' && $name_y eq 'inner fire') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Give a minion huge health... and attack!';   
        }
        
        # combo, small spells
        if (_has_tag($tags_x, 'has_combo', $card_y) && $cost_y <= MIN_SPELL_COST_COMBO_SYNERGY && $type_y eq 'spell') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Small-cost spells means combo-trigger likely.';   
        }        
        ...
        if (_has_tag($tags_x, 'has_adjacentbuff', $card_y) && $cost_y <= MIN_MINION_COST_ADJACENT_BUFF_SYNERGY && $type_y eq 'minion' && !_has_tag($tags_y, 'cursed', $card_x)) { #avoid ancient watcher
            ...
            $reasons{"$name_x|$name_y"} = 'Good size minions means extra damage from adjacent buff.';   
        }
        
        # frostwolf, boardfill
        if (_has_tag($tags_x, 'boardfill', $card_y) && $name_y eq 'frostwolf warlord') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Frostwolf Warlord gets huge with cards that fill the board.';   
        }
        ...
        if ($cost_x <= MIN_MINION_COST_FROSTWOLF_WARLORD_SYNERGY && $type_x eq 'minion' && $name_y eq 'frostwolf warlord') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Frostwolf Warlord grows bigger if you have a lot of minions.';
        }
        # sea giant, minions, implosion
        if (_has_tag($tags_x, 'boardfill', $card_y) && $name_y eq 'sea giant') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Sea Giant can be more easily cast with cards that fill the board.';   
        }
        if ($cost_x <= MIN_MINION_COST_SEAGIANT_SYNERGY && $type_x eq 'minion' && $name_y eq 'sea giant') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Sea Giant costs less mana with more minions on board.';
        }
        ...
        if (_has_tag($tags_x, 'has_deathrattle', $card_y) && $name_x ne 'feugen' && $name_x ne 'stalagg' && !_has_tag($tags_x, 'cursed', $card_y)
            ...
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Return deathrattles to life for extra deathrattles!';   
        }
        
        # secret synergy
        if (_has_tag($tags_x, 'secret_synergy', $card_y) && $text_y =~ 'secret:') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Certain cards have a special synergy with secrets.';   
        }
        
        # shaman overload
        if (_has_tag($tags_x, 'overload', $card_y) && $name_y eq 'unbound elemental') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Unbound Elemental gets bigger with more overload!';   
        }
        ...
        if (_has_tag($tags_x, 'boardfill', $card_y) && $name_y eq 'knife juggler') {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Fill the board for more knives!';   
        }
        
        if (_has_tag($tags_x, 'boardfill', $card_y) && ($name_y eq 'sword of justice' || $name_y eq 'power of the wild')) {
            $g->add_edge($name_x, $name_y, 1.00);
            $reasons{"$name_x|$name_y"} = 'Lots of buffed minions!';   
        }
